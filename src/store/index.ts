...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async SEND_MONEY({ commit, state }, data): Promise<string> {
      ...
      if (firebase.auth().currentUser!.email == data.recipient)
        return '본인에게 송금할 수 없습니다.';
      try {
        // 받는 사람 도큐먼트의 uid 조회
        ...
        if (snapshot.data()!.balance - data.amount < 0)
          return `잔액이 ${Math.abs(snapshot.data()!.balance - data.amount)}원 부족합니다.`;
    ...
    async CHECKOUT({ commit, state }, data): Promise<boolean | string> {
      ...
      try {
        ...
        let newBalance: number =
          Number(snapshot.data()!.balance) - orderDocSnapshot.data()!.totalPrice;
        if (newBalance >= 0) {
          // 결제 가능
          ...
          await transaction(
            '일반 결제',
            orderDocSnapshot.data()!.itemData,
            orderDocSnapshot.data()!.totalPrice
          );
          this.dispatch('GET_BALANCE');
          this.dispatch('GET_TRANSACTIONS');
          return true;
    ...
    async GET_ORDER({ commit, state }, data): Promise<any> {
      ...
      try {
        let orderDocSnapshot = await db.collection('orders').doc(data.orderID).get();
    ...
    async DELETE_ORDER({ commit, state }, data): Promise<boolean> {
      ...
      try {
        await db.collection('orders').doc(data.orderID).delete();
        return true;
      } catch (err) {
    ...
    async GET_SETTING({ commit, state }, data): Promise<any> {
      try {
        let settingsDocSnapshot = await db.collection('settings').doc('settings').get();
