        ...
        <template v-slot:tab2>
          <div class="order__viewpager__numpad">
            ...
            <div class="order__viewpager__numpad__content">
              ...
              <div class="order__viewpager__numpad__content__item" @click="appendTotalStr(0)">
                0
              </div>
      ...
      <ul class="order__content__list">
        <li class="order__content__list__item" v-for="item in selectedList" :key="item.name">
          ...
          <div v-if="item.barcode" class="count">
            <button class="count__action count__action__minous" @click="minousItemCount(item)">
              -
            </button>
            <p>×{{ item.quantity }}</p>
            <button class="count__action count__action__plus" @click="plusItemCount(item)">
              +
            </button>
          </div>
          <p class="price">
            ...
            <span v-if="item.discount"
              >(-{{
                (Number(item.price) * (Number(item.discount || 0) / 100)).numberFormat()
              }})</span
            >
          </p>
